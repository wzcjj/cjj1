...
int **sum1(int n,int **a,int **b,int **c,int **d);//用于处理c11=p3+p4-p1+p6 
int **sum2(int n,int **a,int **b,int **c,int **d); //用于处理c22=p5+p1-p3-p7 
...
int **sum1(int n,int **a,int **b,int **c,int **d)
{  int i,j;
...
int **sum2(int n,int **a,int **b,int **c,int **d)
{  int i,j;
  int **e=new int *[n];for(i=0;i<n;i++) e[i]=new int[n];//定义返回的矩阵 
...
c11=sum1(n/2,p5,p4,p2,p6);
c12=add(n/2,p1,p2);
c21=add(n/2,p3,p4);
c22=sum2(n/2,p5,p1,p3,p7);//高出4小的 
    cout<<"aac"<<endl;
for(int i=0; i<n/2; i++) 
{   
           for(int j=0; j<n/2; j++) 
