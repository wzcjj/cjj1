...
bool cuteasy(int sudoku[][10],int real[][10])
{
	int h=rand()%9,t=rand()%9;//随机生成数独上一个位置 
	while(sudoku[h+1][t+1]==0)//确保该位置非空 
	{
		h=rand()%9;
		t=rand()%9;
	}
	int v=sudoku[h+1][t+1];//v缓存该位置数值 
	...
	sudoku[h+1][t+1]=0; 
	if(cuteasy(sudoku,real))return true;
}
...
bool create(int sudoku[][10],string level)
{
    int NO=0;
	do
	{
	...
	if(level=="easy")cuteasy(sudoku,real);
	else if(level=="normal")cutmid(sudoku,real);
...
int main()
{
    ...
    string level;
    cin>>level;//输入等级，0为easy，1为medium，2为hard 
	create(sudoku,level);
	return 0;
}
