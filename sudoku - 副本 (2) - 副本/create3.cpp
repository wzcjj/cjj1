...
bool create(int sudoku[][10],int level)
{
    int NO=0;
	do
	{
	...
	int sum;
	if(level==0)//简单设定为空少于40个，数独生成率为百分之六十 
	{
		while(cut(sudoku,real))
		{
			sum=0;
			for(int i=1;i<10;++i)for(int j=1;j<10;++j)if(sudoku[i][j]==0)sum++;
			if(sum<40)break;
		}
	}
	else if(level==1)//中等设定为空大于等于40个，少于50个 ，数独生成率为百分之三十八 
	{
		while(cut(sudoku,real))
		{
			sum=0;
			for(int i=1;i<10;++i)for(int j=1;j<10;++j)if(sudoku[i][j]==0)sum++;
			if(sum>=40&&sum<50)break;
		}
	}
	else
	{
		while(cut(sudoku,real))//困难设定为空大于等于50个 ，数独生成率为百分之二 
		{
			sum=0;
			for(int i=1;i<10;++i)for(int j=1;j<10;++j)if(sudoku[i][j]==0)sum++;
			if(sum>=50)break;
