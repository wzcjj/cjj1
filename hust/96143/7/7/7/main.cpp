...
public:
    ...
    void build(int l, int r, int n, int p = 1){
        tree[p].l = l;
        tree[p].r = r;
        if(l == r)tree[p].seg.build(1, n);
        else tree[p].seg.build(1, n);
        if(l < r){
            int mid = (l + r) >> 1;
            build(l, mid, n, 2 * p);
...
int main(int argc, const char * argv[]) {
    ...
    while (N--) {
        ...
        for (i=1; i<=m; i++) {
            int x,y,z;
            scanf("%d%d%d",&x,&y,&z);
            int ret = (seg.querymax(max(x-z/2, 1), min(x+z/2, n), max(y-z/2, 1), min(y+z/2, n)) + seg.querymin(max(x-z/2, 1), min(x+z/2, n), max(y-z/2, 1), min(y+z/2, n)))/2;
            printf("%d\n",ret);
            seg.modify(x, x, y, y, ret);
        }
    }
    return 0;
}
